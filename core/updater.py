...
def updater():
    ...
    latestCommit = get(
        'https://raw.githubusercontent.com/s0md3v/XSSniper/master/core/config.py').text
    ...
    if changes not in latestCommit:  # just a hack to see if a new version is available
        ...
        logger.good('A new version of XSSniper is available.')
        changes_str = 'Changes:\n'
        ...
        if choice != 'n':
            logger.run('Updating XSSniper')
            os.system(
                'git clone --quiet https://github.com/s0md3v/XSSniper %s' % (folder))
            os.system('cp -r %s/%s/* %s && rm -r %s/%s/ 2>/dev/null' %
                      (path, folder, path, path, folder))
            logger.good('Update successful!')
    else:
        logger.good('XSSniper is up to date!')
